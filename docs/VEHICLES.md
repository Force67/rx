...
- **Rotational aero damping** (`roll_damp` / `pitch_damp` / `yaw_damp`) — real
  airframes damp their own rotation strongly; the collision box excludes the
  wings so Jolt's inertia is small, and these keep rates physical and post-stall
  tumbling bounded.
...
`nose_steer_angle_rad` is full low-speed nose-wheel deflection.
...
`total_mass_kg`, `over_mtom`, `position`, `rotation`. `set_wind(Vec3)` adds a
uniform airmass velocity (defaults to still air).
...
to `VehicleAudio` (adds `position` for panning and `submerged` for ducking);
`SynthParams` is what actually reaches the oscillators (`rpm`, `load`,
`throttle`, `speed_mps`, `slip`). Values are clamped on use, so a rough estimate
is fine. Per vehicle kind:

| Source | → SynthParams / VehicleAudioState |
| --- | --- |
| Car `VehicleState` | `rpm←rpm`, `load←engine_load`, `throttle←input`, `speed←forward_speed`, `slip←max wheel longitudinal_slip` |
| Boat `BoatState` | `rpm←rpm`, `load←engine_load`, `throttle←max(0, throttle)`, `speed←speed_mps`, `submerged←!prop_submerged` |
| Aircraft `AircraftState` | `rpm←rpm` (prop rpm or N1 %), `load←engine_load`, `throttle←throttle`, `speed←airspeed_mps` |
...
- **Raycast body-exclusion** — `PhysicsWorld::Raycast` has no body filter, so an
  aircraft's gear suspension rays would hit its own fuselage; gear attach points
  must sit **below** the collision box to clear it.
- **No custom inertia tensor API** — the aircraft collision box excludes the
  wings, so Jolt's inertia is small; the rotational aero damping terms
  compensate rather than a real inertia override.
- **No wind field** — `Aircraft::set_wind` is a one-line hook for a uniform
  airmass velocity, defaulted to zero; there is no spatial wind field yet.
- **Audio telemetry wishlist** — per-wheel slip (only an aggregate reaches
  `SynthParams` today), explicit gear/clutch flags, and a jet spool-vs-thrust
  split would let the synths react more precisely.
