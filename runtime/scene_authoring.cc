...
struct SurfaceMap {
  ...
  // for the same reason `srgb` does: the file itself says nothing about how it
...
std::string BindSurfaceTexture(const std::string& file, bool srgb, asset::TextureRole role,
  ...
  if (!db.FindTexture(id)) {
    ...
    // Between the decode and the upload, because MaterialSystem cannot build a
    // mip chain for a block format and the compressor is what bakes one.
    asset::CompressTexture(&texture, role, file);
...
std::string ApplyMaterialXMaps(const asset::MaterialXMaps& maps, asset::AssetDatabase& db,
  ...
  const struct {
    ...
    asset::TextureRole role;
      ...
      {maps.base_color, &asset::Material::base_color, true, asset::TextureRole::kColor},
      {maps.normal, &asset::Material::normal, false, asset::TextureRole::kNormalTangent},
      {maps.roughness, &asset::Material::metallic_roughness, false, asset::TextureRole::kData},
      {maps.metallic, &asset::Material::metallic_map, false, asset::TextureRole::kData},
      {maps.occlusion, &asset::Material::occlusion_map, false, asset::TextureRole::kData},
      {maps.emissive, &asset::Material::emissive, true, asset::TextureRole::kColor},
  };
...
bool ApplyPattern(const ScenePattern& pattern, const std::string& key, asset::AssetDatabase& db,
  ...
  // Generated maps compress like any other: the synthesis is unchanged, the
  // block encode sits between it and the upload. Two-tone patterns are the
  // easy case for BC (a block spanning an edge is still two colours on one
  // line), and Pattern is what most shipped scenes texture with, so leaving
  // these at rgba8 would mean the saving never reached them.
