...
base::Option<double> FroxelDensity{"froxel.density", 0.005, "RX_FROXEL_DENSITY"};
...
base::Option<double> HdrPaperWhite{"hdr.paper.white", 200.0, "RX_HDR_PAPER_WHITE"};
...
base::Option<int> HdrForceTransfer{"hdr.force.transfer", 0, "RX_HDR_FORCE_TRANSFER"};
...
base::Option<bool> MeshShaderLod{"mesh.shader.lod", false, "RX_MESH_SHADER_LOD"};
...
base::Option<bool> PathtraceReference{"pathtrace.reference", false, "RX_PATHTRACE_REFERENCE"};
...
base::Option<bool> PathtraceRecon{"pathtrace.recon", false, "RX_PATHTRACE_RECON"};
base::Option<int> PathtraceReconDebug{"pathtrace.recon.debug", 0, "RX_PATHTRACE_RECON_DEBUG"};
base::Option<bool> PathtraceRestir{"pathtrace.restir", true, "RX_PATHTRACE_RESTIR"};
base::Option<bool> PathtraceRestirDi{"pathtrace.restir.di", true, "RX_PATHTRACE_RESTIR_DI"};
...
bool Renderer::Initialize(const RendererDesc& desc, Window& window) {
  ...
  if (const char* name = RhiBackend.get()) {
    ...
    else RX_WARN("RX_RHI: unknown backend '{}', using {}", value, BackendName(backend));
  }
  ...
  if (settings_.hdr_output && !swapchain_hdr_request_) {
    RX_INFO("hdr output requested but the system is not compositing in hdr; using sdr "
  ...
  if (const char* spec = Sequence.get()) {
    ...
    if (fields.size() >= 3) {
      ...
      RX_WARN("RX_SEQ ignored, expected prefix:startsec:count[:stride], got '{}'", spec);
    }
  }

  // RX_HDR=/tmp/frame.hdr:12 exports the linear-hdr frame (radiance rgbe) at t=12s.
  ...
  if (mesh_shader_ok) {
    RX_INFO("mesh-shader lod path available (default {})", settings_.mesh_shader_lod ? "on" : "off");
  } else {
    if (settings_.mesh_shader_lod) {
      RX_WARN("mesh-shader lod requested but unavailable on this gpu, disabling");
    }
  ...
  if (SkinDynamicsOpt.overridden()) settings_.skin_dynamics = SkinDynamicsOpt;
  ...
  // RX_AERIAL overrides aerial-perspective strength (0 off, 1 physical, >1 exaggerated).
...
void Renderer::WriteHdr() {
  ...
  if (ok) {
    RX_INFO("{} frame written: {} ({}x{})", is_exr ? "exr" : "hdr", hdr_path_, hdr_width_,
             hdr_height_);
  } else {
...
void Renderer::ApplySettings() {
  ...
  if (want_msaa != applied_msaa_samples_ && mesh_pipeline_) {
    ...
    if (rebuilt) {
      ...
      RX_WARN("msaa: mesh pipeline rebuild failed, keeping {}x", applied_msaa_samples_);
...
void Renderer::RenderFrame(const FrameView& view) {
  ...
  if (settings_.frame_generation && !settings_.path_trace &&
    ...
    if (!framegen_ && !framegen_attempted_) {
      ...
      if (!framegen_) RX_WARN("framegen: unavailable, presenting real frames only");
    }
    if (framegen_) {
...
void Renderer::BuildFrameGraph(FrameResources& frame, u32 image_index, const FrameView& view) {
  ...
  if (settings_.sss) {
    ...
    auto fill_sss = [this, proj](SssPush& p) {
      ...
      p.max_radius = 24.0f;
      p.strength = std::getenv("RX_SSS_DEBUG") ? -1.0f : 1.0f;  // <0 = mask debug view
    };
