...
enum class BarrierScope : u8 {
  ...
  kComputeReadWrite,  // compute shader reads AND storage writes (e.g. a cleared
                      // buffer consumed by atomics that both load and store)
...
// is compiled in (RX_RHI_D3D12) every shader also carries a DXIL
...
// blob. With RX_RHI_D3D12 the embed step emits a k_<sym>_dxil sidecar
