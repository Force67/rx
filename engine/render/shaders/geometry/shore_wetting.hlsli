...
// frame.shore_field: xy field min-corner world xz, z 1/extent (1/m), w unused.
// Returns 0..1; the field's own texel filtering keeps the boundary a soft band.
float ShoreWetness(float3 world_pos, uint flags, float4 shore_field) {
  ...
  return saturate(shore_wetness_map.SampleLevel(shore_wetness_sampler, uv, 0.0));
}
