#ifndef RX_GI_RCGI_CACHE_SHADE_BODY_HLSLI_
#define RX_GI_RCGI_CACHE_SHADE_BODY_HLSLI_

#include "rhi_bindings.hlsli"
#include "gi/rcgi_common.hlsli"
#include "gi/light_grid.hlsli"
// Shared body for the RCGI cache-shade pass. Indirect dispatch over the active-
// cell list the probe trace built (one thread per stale entry). Lights the hit
// with sun + emissive + point/spot lights from the world light grid + a
// previous-frame bounce from the irradiance cascades, and stores the HDR
// radiance into the cache. Amortised: each unique world cell is shaded once.
//
// Two seams select between hardware and software (RCGI_TRACE_SDF):
//   - surface: hardware re-resolves the hit triangle/material through the
//     bindless scene tables; software reads albedo/emissive the SDF probe trace
//     already packed into the entry (slots 1/2, see rcgi_common.hlsli).
//   - sun visibility: hardware casts a TLAS shadow ray; software sphere-traces
//     the SDF clipmap toward the sun (binary occlusion). Neither light-grid nor
//     irradiance-bounce paths use ray query, so they are identical.
// The software variant emits no RayQuery capability, so it creates on non-ray-
// query devices.
...
#else
// SDF clipmap trace seam (set 1) for the sun occlusion trace.
...
void main(uint3 id : SV_DispatchThreadID) {
  ...
  // rather than issue an out-of-bounds bindless fetch. (Hardware only: the
  // software branch reads no bindless tables, so there is nothing to bound.)
  ...
  float3 radiance = albedo / kRcgiPi * sun * ndl * shadow + emissive;

  // Point/spot lights from the world light grid (no ray query).
  uint flat_cell;
...
#endif  // RX_GI_RCGI_CACHE_SHADE_BODY_HLSLI_
