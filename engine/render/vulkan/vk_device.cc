...
VKAPI_ATTR VkBool32 VKAPI_CALL DebugCallback(VkDebugUtilsMessageSeverityFlagBitsEXT severity,
  ...
  if (severity >= VK_DEBUG_UTILS_MESSAGE_SEVERITY_ERROR_BIT_EXT) {
    RX_ERROR("vulkan: {}", data->pMessage);
  } else {
    RX_WARN("vulkan: {}", data->pMessage);
  }
  return VK_FALSE;
}
...
std::unique_ptr<Device> VulkanDevice::CreateImpl(const DeviceDesc& desc, Window* window) {
  ...
  if (validation) {
    ...
    RX_WARN("validation requested but layer not installed");
  }
  ...
  const char** ngx_device_exts = nullptr;
  {
    ...
    if (NVSDK_NGX_VULKAN_RequiredExtensions(&ngx_instance_ext_count, &ngx_instance_exts,
      ...
      RX_WARN("dlss: required extension query failed, dlss will be unavailable");
    }
  }
#endif
  ...
  if (vkCreateInstance(&instance_info, nullptr, &device->instance_) != VK_SUCCESS) {
    RX_ERROR("vkCreateInstance failed");
    return nullptr;
  }
  ...
  if (vkCreateDevice(device->physical_device_, &device_info, nullptr, &device->device_) !=
      VK_SUCCESS) {
    RX_ERROR("vkCreateDevice failed");
    return nullptr;
  }
  ...
  if (device->caps_.async_compute) {
    if (dedicated_compute) {
      ...
      RX_INFO("async compute: dedicated queue family {}", device->compute_family_);
    } else {
  ...
  // the driver, so loading is fire-and-forget.
  {
    ...
    if (!blob.empty()) RX_INFO("pipeline cache: loaded {} KB", blob.size() >> 10);
  }
...
bool VulkanDevice::InitResources() {
  ...
  if (vmaCreateAllocator(&info, &allocator_) != VK_SUCCESS) {
    RX_ERROR("vma allocator creation failed");
    return false;
  }
...
GpuBuffer VulkanDevice::CreateBuffer(u64 size, BufferUsageFlags usage, bool host_visible) {
  ...
  if (vmaCreateBuffer(allocator_, &buffer_info, &alloc_info, &buffer, &allocation, &result) !=
      VK_SUCCESS) {
    RX_ERROR("buffer allocation failed ({} bytes)", size);
    return {};
  }
...
VmaAllocationCreateInfo alloc_info{};
  ...
  if (result != VK_SUCCESS) {
    RX_ERROR("image allocation failed: result {} format {} {}x{} usage {:#x} mips {}",
...
VmaAllocationCreateInfo alloc_info{};
  ...
  if (vmaCreateImage(allocator_, &image_info, &alloc_info, &image, &allocation, nullptr) !=
      VK_SUCCESS) {
    RX_ERROR("3d image allocation failed: format {} {}x{}x{}", static_cast<int>(format), width,
              height, depth);
    return {};
  }
...
VmaAllocationCreateInfo alloc_info{};
  ...
  if (vmaCreateImage(allocator_, &image_info, &alloc_info, &image, &allocation, nullptr) !=
      VK_SUCCESS) {
    RX_ERROR("cube image allocation failed");
    return {};
  }
...
SamplerHandle VulkanDevice::GetSampler(const SamplerDesc& desc) {
  ...
  if (vkCreateSampler(device_, &info, nullptr, &sampler) != VK_SUCCESS) {
    RX_ERROR("sampler creation failed");
    return {};
  }
...
VkDescriptorSetLayout VulkanDevice::GetOrCreateSetLayout(const BindingLayoutDesc& desc) {
  ...
  if (vkCreateDescriptorSetLayout(device_, &info, nullptr, &layout) != VK_SUCCESS) {
    RX_ERROR("descriptor set layout creation failed");
...
VkPipelineLayout VulkanDevice::GetOrCreatePipelineLayout(
  ...
  if (vkCreatePipelineLayout(device_, &info, nullptr, &layout) != VK_SUCCESS) {
    RX_ERROR("pipeline layout creation failed");
    return VK_NULL_HANDLE;
  }
...
BindingSetHandle VulkanDevice::CreateBindingSet(BindingLayoutHandle layout, u32 variable_count) {
  ...
  if (vkCreateDescriptorPool(device_, &pool_info, nullptr, &pool) != VK_SUCCESS) {
    RX_ERROR("binding set pool creation failed");
    return {};
  }
  ...
  if (vkAllocateDescriptorSets(device_, &alloc, &set) != VK_SUCCESS) {
    RX_ERROR("binding set allocation failed");
...
bool VulkanDevice::BuildComputePipeline(const ComputePipelineDesc& desc,
  ...
  if (module == VK_NULL_HANDLE) {
    RX_ERROR("compute shader module creation failed ({})",
  ...
  if (result != VK_SUCCESS) {
    RX_ERROR("compute pipeline creation failed ({})", desc.debug_name ? desc.debug_name : "?");
    return false;
  }
...
bool VulkanDevice::BuildGraphicsPipeline(const GraphicsPipelineDesc& desc,
  ...
  if (!ok) {
    destroy_modules();
    RX_ERROR("graphics shader module creation failed ({})",
              desc.debug_name ? desc.debug_name : "?");
    return false;
  }
  ...
  destroy_modules();
  if (result != VK_SUCCESS) {
    RX_ERROR("graphics pipeline creation failed ({})", desc.debug_name ? desc.debug_name : "?");
    return false;
  }
...
AccelStructHandle VulkanDevice::CreateAccelStruct(AccelStructType type, u64 size) {
  ...
  if (vmaCreateBuffer(allocator_, &buffer_info, &alloc_info, &backing.buffer,
    ...
    RX_ERROR("acceleration structure backing allocation failed ({} bytes)", size);
    return {};
  }
  ...
  if (vkCreateAccelerationStructureKHR(device_, &info, nullptr, &accel) != VK_SUCCESS) {
    ...
    RX_ERROR("acceleration structure creation failed");
    return {};
  }
...
void VulkanDevice::SubmitAsync(CommandList*) {
  ...
  if (!logged) {
    logged = true;
    RX_INFO("async compute: first submission (family {})", compute_family_);
  }
}
