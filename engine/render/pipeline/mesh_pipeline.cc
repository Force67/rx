...
std::unique_ptr<MeshPipeline> MeshPipeline::Create(Device& device, Format color_format,
  ...
  // vertex stage + the bone weight stream. Wireframe is not skinned.
  {
    ...
    for (u32 variant = 0; variant < 2; ++variant) {
      ...
      if (!pipeline->skinned_pipelines_[variant]) {
        RX_ERROR("mesh skinned pipeline creation failed (variant {})", variant);
      }
    }
  }
  ...
  // depth without writing, shade with the same pbr shaders.
  {
    ...
    for (u32 variant = 0; variant < 2; ++variant) {
      ...
      desc.fragment = variant == 1 ? RX_SHADER(k_mesh_rt_ps_hlsl) : RX_SHADER(k_mesh_ps_hlsl);
      pipeline->blend_pipelines_[variant] = device.CreateGraphicsPipeline(desc);
      if (!pipeline->blend_pipelines_[variant]) {
        RX_ERROR("mesh blend pipeline creation failed");
      }
    }
    ...
    for (u32 variant = 0; variant < 2; ++variant) {
      ...
      if (!pipeline->blend_additive_pipelines_[variant]) {
        RX_ERROR("mesh additive blend pipeline creation failed");
      }
    }
  }
  ...
  // Prepass: depth write + normals/motion/depth-export targets, same layout.
  {
    ...
    if (!pipeline->prepass_pipeline_) {
      RX_ERROR("mesh prepass pipeline creation failed");
    }
    ...
    if (!pipeline->skinned_prepass_pipeline_) {
      RX_ERROR("mesh skinned prepass pipeline creation failed");
    }
    ...
    if (!pipeline->skinned_prepass_masked_pipeline_) {
      RX_ERROR("mesh skinned masked prepass pipeline creation failed");
    }
  }
  ...
  if (mesh_caps) {
    ...
    // motion target, like the raster main variants.
    {
      ...
      for (u32 variant = 0; variant < 2; ++variant) {
        ...
        if (!pipeline->ms_scene_[variant]) {
          RX_ERROR("mesh-shader scene pipeline creation failed (variant {})", variant);
        }
      }
    }
    ...
    // applies to the raster prepass, which bins per submesh.
    {
      ...
      if (!pipeline->ms_prepass_) {
        RX_ERROR("mesh-shader prepass pipeline creation failed");
      }
    }
  }
