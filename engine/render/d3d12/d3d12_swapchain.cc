...
// RX_UI_SHOT screenshot readback) runs unchanged against the ring.
...
bool D3D12Swapchain::Init(u32 width, u32 height, bool vsync) {
  ...
  if (FAILED(CreateDXGIFactory2(0, IID_IDXGIFactory4, reinterpret_cast<void**>(&factory)))) {
    RX_ERROR("d3d12: dxgi factory creation failed");
    return false;
  }
  ...
  factory->Release();
  if (FAILED(hr)) {
    RX_ERROR("d3d12: swapchain creation failed");
    return false;
  }
  hr = swapchain1->QueryInterface(IID_IDXGISwapChain3, reinterpret_cast<void**>(&dxgi_));
  swapchain1->Release();
  if (FAILED(hr)) {
    RX_ERROR("d3d12: IDXGISwapChain3 unavailable");
    return false;
  }
  ...
  for (u32 i = 0; i < desc.BufferCount; ++i) {
    ...
    if (FAILED(dxgi_->GetBuffer(i, IID_ID3D12Resource, reinterpret_cast<void**>(&buffer)))) {
      RX_ERROR("d3d12: swapchain buffer {} unavailable", i);
      return false;
    }
  ...
  RX_INFO("swapchain: {}x{}, {} images, dxgi flip-model ({})", extent_.width, extent_.height,
...
bool D3D12Swapchain::Init(u32 width, u32 height, bool vsync) {
  ...
  for (u32 i = 0; i < kImageCount; ++i) {
    GpuImage image = device_.CreateImage2D(
        ...
        1, 1);
    if (!image) {
      RX_ERROR("d3d12: offscreen swapchain image creation failed");
      return false;
    }
    images_.push_back(image);
  }
  RX_INFO("swapchain: {}x{}, {} images, offscreen (no display path through vkd3d)",
