...
bool BloomPass::Initialize(Device& device) {
  ...
  if (!make(RX_SHADER(k_bloom_down_cs_hlsl), "bloom_down", &down_pipeline_) ||
      !make(RX_SHADER(k_bloom_up_cs_hlsl), "bloom_up", &up_pipeline_)) {
    RX_ERROR("bloom pipeline creation failed");
    return false;
  }
  return true;
}
