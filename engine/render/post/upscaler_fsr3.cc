 ...
 public:
  ...
  bool Initialize(const UpscalerDesc& desc) override {
    ...
    FfxErrorCode err = ffxGetInterfaceVK(&interface_, ffx_device, scratch_, scratch_size_,
                                         FFX_FSR3UPSCALER_CONTEXT_COUNT);
    if (err != FFX_OK) {
      RX_ERROR("fsr3: backend interface creation failed ({})", static_cast<int>(err));
      return false;
    }
    ...
    if (err != FFX_OK) {
      RX_ERROR("fsr3: context creation failed ({})", static_cast<int>(err));
      return false;
    }
    context_valid_ = true;
    ...
    RX_INFO("fsr3 upscaler ready: {}x{} -> {}x{}", desc.render_width, desc.render_height,
 ...
 private:
  ...
  bool CreateSharedResources() {
    ...
    if (ffxFsr3UpscalerGetSharedResourceDescriptions(&context_, &shared) != FFX_OK) {
      RX_ERROR("fsr3: shared resource descriptions query failed");
      return false;
    }
    ...
    for (u32 i = 0; i < kSharedCount; ++i) {
      ...
      if (!shared_[i]) {
        RX_ERROR("fsr3: shared resource allocation failed");
        return false;
      }
      shared_descs_[i] = res;
    }
