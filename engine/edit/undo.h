...
class RX_EDIT_EXPORT UndoStack {
 public:
  // Applies `cmd` immediately, then records it (clearing the redo stack). Inside
  // a group the command is applied and buffered into the group.
  ...
  // step labelled `label`. Groups may nest; only the outermost boundary records.
  void BeginGroup(const char* label);
  void EndGroup();

  bool can_undo() const { return !undo_.empty(); }
  bool can_redo() const { return !redo_.empty(); }
  const char* undo_label() const { return undo_.empty() ? "" : undo_.back()->label(); }
  const char* redo_label() const { return redo_.empty() ? "" : redo_.back()->label(); }
...
RX_EDIT_EXPORT std::unique_ptr<Command> MakeSetProp(ecs::World& world, ecs::Entity entity,
                                                    const ComponentDesc& comp, const PropDesc& prop,
                                                    PropValue new_value);

// Creates an entity with the given components and initial prop values. The
// entity's Guid is stable across undo/redo; *out_entity receives the live handle
// on each Apply (may be null).
...
RX_EDIT_EXPORT std::unique_ptr<Command> MakeDestroyEntity(ecs::World& world, ecs::Entity entity);

// Reparents an entity, preserving its world transform. An invalid new_parent
// unparents it.
RX_EDIT_EXPORT std::unique_ptr<Command> MakeReparent(ecs::World& world, ecs::Entity entity,
...
RX_EDIT_EXPORT std::unique_ptr<Command> MakeAddComponent(ecs::World& world, ecs::Entity entity,
                                                         const ComponentDesc& comp);

// Removes a component; snapshots its props so Revert restores them.
RX_EDIT_EXPORT std::unique_ptr<Command> MakeRemoveComponent(ecs::World& world, ecs::Entity entity,
                                                            const ComponentDesc& comp);
