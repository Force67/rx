...
bool AudioDevice::Open(Mixer* mixer) {
  ...
  if (!SDL_InitSubSystem(SDL_INIT_AUDIO)) {
    RX_WARN("audio: SDL audio init failed: {}", SDL_GetError());
    return false;
  }
  SDL_AudioSpec spec;
  ...
  if (!stream) {
    RX_WARN("audio: no playback device: {}", SDL_GetError());
  ...
  active_ = true;
  RX_INFO("audio: device open, {} Hz stereo f32", kMixRate);
  return true;
}
...
bool AudioDevice::Open(Mixer* mixer) {
  ...
  RX_INFO("audio: built without SDL audio, running silent");
  return false;
}
