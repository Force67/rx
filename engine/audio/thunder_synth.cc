...
public:
  ThunderDecoder(u32 rate, u32 seed, f32 energy, f32 distance_m)
      : rate_(static_cast<f32>(rate)), rng_(seed ? seed : 0x9e3779b9u) {
    ...
    f32 cutoff_hz = 2600.0f - 2250.0f * muffle; // 2.6 kHz close -> 350 Hz far
    body_lp_k_ = 1.0f - std::exp(-2.0f * 3.14159265f * cutoff_hz / rate_);

    seconds_ = 5.0f + 4.0f * energy_;
    total_frames_ = static_cast<u64>(seconds_ * rate_);
  ...
  u32 sample_rate() const override { return static_cast<u32>(rate_); }
  ...
  u32 Read(float *out, u32 frames) override {
    ...
    for (u32 i = 0; i < n; ++i) {
      f32 t = static_cast<f32>(frame_ + i) / rate_;
      f32 white = Noise();
      ...
      for (int b = 0; b < 4; ++b) {
        ...
        if (dt_b > 0.0f)
          env += bump_w_[b] * dt_b * std::exp(1.0f - dt_b * 1.6f) * 1.6f;
      }
      env *= std::exp(-t / (2.2f + 2.0f * energy_));
      wobble_ += 0.00015f * (Noise() - wobble_);
      env *= 1.0f + wobble_ * 40.0f;
      f32 body = body_lp_ * env * (14.0f + 10.0f * energy_);
